...
class TimeSformer(nn.Module):
    def __init__(
        ...
        for _ in range(depth):
            ...
            channel_attn = Attention(dim, dim_head=dim_head, heads=heads, dropout=attn_dropout)

            if shift_tokens:
                time_attn, spatial_attn, channel_attn, ff = map(lambda t: PreTokenShift(num_frames, t),
                                                                (time_attn, spatial_attn, channel_attn, ff))

            time_attn, spatial_attn, channel_attn, ff = map(lambda t: PreNorm(dim, t),
                                                            (time_attn, spatial_attn, channel_attn, ff))

            self.layers.append(nn.ModuleList([time_attn, spatial_attn, channel_attn, ff]))
    ...
    def forward(self, video, mask=None):
        b, f, c, h, w, *_, device, p = *video.shape, video.device, self.patch_size
        ...
        # time, space and channel attention
        for (time_attn, spatial_attn, channel_attn, ff) in self.layers:
            # Time attention
            x = time_attn(x, 'b (f n) d', '(b n) f d', n=n, f=f,
                          mask=frame_mask, cls_mask=cls_attn_mask, rot_emb=frame_pos_emb) + x

            # Spatial attention
            x = spatial_attn(x, 'b (f n) d', '(b f) n d', f=f,
                             cls_mask=cls_attn_mask, rot_emb=image_pos_emb) + x

            # Channel attention
            x = channel_attn(x, 'b n d', 'b n d') + x
...
class Attention(nn.Module):
    ...
    def forward(self, x, einops_from, einops_to, mask=None, cls_mask=None, rot_emb=None, **einops_dims):
        ...
        # rearrange across time or space or channel
        ...
        # 根据注意力类型处理cls token
        if 'f n' in einops_from and 'f d' in einops_to:  # time attention
            cls_k = cls_k.repeat_interleave(einops_dims['n'], dim=0)
            cls_v = cls_v.repeat_interleave(einops_dims['n'], dim=0)
        elif '(b f)' in einops_to:  # spatial attention
            cls_k = cls_k.repeat_interleave(einops_dims['f'], dim=0)
            cls_v = cls_v.repeat_interleave(einops_dims['f'], dim=0)
